# VetPlus — For Development Plan (Authoritative Spec)

This document defines the end-to-end product and technical plan for the VetPlus mobile app purchased by veterinary clinics. It covers roles, features, data model, security, UX, delivery, and acceptance criteria, aligned with the current Flutter + Firebase implementation in `apps/mobile-flutter`.
...
M4.1 Account Creation & Invite Flows (No-backend, finalized)

- Goals: Keep Firebase on Spark (free), avoid server code, but deliver a clear flow for creating clinics, clinic admins, and vets.
- Core pattern: Use placeholder Firestore user docs for invites (`temp_admin_<sanitized_email>`, `temp_vet_<sanitized_email>`). On first verified login with that email, the client links the real Auth user to the placeholder and deletes the temporary doc.

- App Owner → Create Clinic + Primary Admin

  - Create `clinics/{id}` with `adminId = temp_admin_<sanitized_email>`.
  - Create `users/temp_admin_...` with `userType=clinicAdmin`, `clinicRole=admin`, `connectedClinicId`.
  - Owner communicates instructions to the admin: sign up and verify. On first login, the app updates `clinics/{id}.adminId` to the real `uid`, moves the member record, and deletes the temp user.

- App Owner → Add/Remove Extra Clinic Admins

  - Add: Use the Manage Admins UI (existing user only). For non-existing emails, send a password reset from the UI so they can claim the account; or create a clinic for them (which seeds a temp admin).
  - Remove: Delete `clinics/{id}/members/{uid}` and clear `users/{uid}.connectedClinicId/clinicRole`. If removing the primary admin, appoint a replacement before updating `clinics/{id}.adminId`.

- Clinic Admin → Add Vets

  - Add vet by email. If the email exists in `users`, attach directly; if not, create a `temp_vet_*` profile so first login will auto-link.
  - New UI action: “Send Password Reset” next to the email field, which triggers `FirebaseAuth.instance.sendPasswordResetEmail(email)`. This helps when the email already has an Auth account.

- Passwords & First Login

  - Users set their own password at sign-up, or receive a reset email. We do not set passwords for other users from the client (no backend).
  - Optionally, an app-level `mustChangePassword` flag can gate the dashboard until a password update, but it is not enforced by Firebase Auth.

- Security & Rules
  - Emails are normalized to lowercase for matching.
  - Rules include helpers to allow one-time linking: placeholder admin IDs and the real admin may update `clinics/{id}.adminId` safely.
  - All placeholder IDs match `^_?temp_(admin|account|vet)_.+`.

Outcome: A robust, $0 backend cost flow that supports creating clinics/admins and adding vets, with a built-in “Send Password Reset” path to handle existing Auth accounts.

M4.2 Auth Email Templates & Localization (Level 1)

- Purpose: Customize Firebase Auth emails (password reset, verification) without adding a backend.
- Implementation steps:
  1. In the Firebase console, go to **Authentication → Templates**. Edit the password reset (and verification, if needed) template: set subject, body (use placeholders like `{{email}}`, `{{url}}`), logo, accent color, and reply address.
  2. For localization, enable the desired languages in the same screen. Firebase will auto-select the language that matches the client setting.
  3. The Flutter app sets the auth language at startup (see `main.dart`), using the device locale via `FirebaseAuth.instance.setLanguageCode(...)`.
  4. If you need to force a specific language (e.g., override per clinic), call `setLanguageCode('pt')` (BCP‑47 code).
- Notes:
  - Template edits are project-wide; per-clinic branding would require the “Level 2” backend approach.
  - The reset emails include the Firebase-generated link (optionally a Dynamic Link if configured in Auth settings).
  - TODO: Schedule console template customization (copy, logo, colors) before launch.
